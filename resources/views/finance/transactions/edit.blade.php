<x-layouts.main>
    <div class="animate-fade-in max-w-3xl mx-auto">
        ...
        <div class="mb-6 sm:mb-8">
            ...
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Edit Transaksi</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm sm:text-base">Perbarui detail transaksi</p>
        </div>
        ...
        <div class="card-mobile">
            <form method="POST" action="{{ route('transactions.update', $transaction) }}">
                ...
                <!-- Type Selection - Mobile First -->
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Tipe Transaksi</label>
                    <div class="grid grid-cols-2 gap-3 sm:gap-4">
                        <label class="relative cursor-pointer">
                            <input type="radio" name="type" value="income" required class="peer sr-only" {{ $transaction->type === 'income' ? 'checked' : '' }}>
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="type" value="expense" required class="peer sr-only" {{ $transaction->type === 'expense' ? 'checked' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    >
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ $transaction->category_id == $category->id ? 'selected' : '' }}>{{ $category->name }} ({{ ucfirst($category->type) }})</option>
                        @endforeach
                    </select>
                </div>
